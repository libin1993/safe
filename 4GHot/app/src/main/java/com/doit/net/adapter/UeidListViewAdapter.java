...
import com.doit.net.model.DBCity;
...
public class UeidListViewAdapter extends BaseSwipeAdapter {
    ...
    private void checkBlackWhiteList(UeidBean resp, TextView tvContent) {
        ...
        if (CacheManager.currentWorkMode.equals("2")) {
            String content = "IMSI：" + resp.getImsi() + "          " + "制式: ";
            try {
                if (!"".equals(resp.getImsi())) {
                    ...
                    String phoneNumber = FormatUtils.getInstance().getPhoneNumber(resp.getImsi());

                    if (!TextUtils.isEmpty(phoneNumber)) {
                        DBCity city = dbManager.selector(DBCity.class).where("number", "=", phoneNumber).findFirst();
                    ...
                    content += cityName + UtilOperator.getOperatorNameCH(resp.getImsi()) + "\n"
                            + resp.getRptTime() + "       " + "次数：" + resp.getRptTimes() + "       "
                            + mContext.getString(R.string.ueid_last_intensity) + resp.getSrsp();

                    WhiteListInfo info = dbManager.selector(WhiteListInfo.class).where("imsi", "=", resp.getImsi()).findFirst();
                    if (info != null) {
                        msisdn = info.getMsisdn();
