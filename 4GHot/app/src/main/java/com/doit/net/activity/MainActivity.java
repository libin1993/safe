...
//    private final int SYS_RPT = 1;
    ...
    private void initDB() {
        ...
        try {
            long count = dbManager.selector(DBCity.class).count();
            if (count == 0) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            InputStream is = getResources().getAssets().open("city.txt");
                            BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(is));
                            String readLine = "";
                            List<DBCity> cityList = new ArrayList<>();
                            while ((readLine = bufferedReader.readLine()) != null) {
                                String[] split = readLine.split("\t");
                                cityList.add(new DBCity(split[0], split[2]));
                            }
                            is.close();
